...
const String requestId = 'requests';
...
class RequestsBloc extends Bloc<RequestsEvent, RequestsState> {
  ...
  Future _onGet(_Get event, Emitter<RequestsState> emit) async {
    ...
    final cachedRequests = await databaseRepository.fetchRequests(
      searchTerm: effectiveSearchTerm,
    );

    emit(state.copyWith(chats: cachedRequests));

    // Fetch fresh data from server
    if (!webSocketService.isConnected) {
      emit(state.copyWith(status: RequestsStatus.failure));
      return;
    }
  ...
  Future _onReceived(_Received event, Emitter<RequestsState> emit) async {
    try {
      if (event.payload['response_status'] == 200) {
        ...
        final chats = await databaseRepository.fetchRequests(
  ...
  Future _onUpdate(_Update event, Emitter<RequestsState> emit) async {
    ...
    try {
      ...
      emit(state.copyWith(chats: chats, status: RequestsStatus.success));
    } catch (error, stackTrace) {
      addError(error, stackTrace);
      emit(state.copyWith(status: RequestsStatus.failure));
    }
