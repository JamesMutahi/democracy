...
class MessagesBloc extends Bloc<MessagesEvent, MessagesState> {
  ...
  Future _onReceived(_Received event, Emitter<MessagesState> emit) async {
    ...
    try {
      if (event.payload['response_status'] == 200) {
        ...
        for (Map<String, dynamic> data in event.payload['data']['results']) {
          await databaseRepository.saveMessageData(data: data);
        }
  ...
  Future _onUpdate(_Update event, Emitter<MessagesState> emit) async {
    ...
    emit(state.copyWith(messages: messages, status: MessagesStatus.success));
  }
